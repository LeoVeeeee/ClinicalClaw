"""Citation-based answer generation baseline."""

from __future__ import annotations

from clinicalclaw.llm import DeterministicLLMProvider
...
class CitationAnswerGenerator:
    ...
    def __init__(self, llm: DeterministicLLMProvider | None = None) -> None:
    ...
    def generate(
        ...
        A deployed research system would prompt an LLM with evidence passages.
        The baseline keeps this deterministic and easy to test.
        """
        ...
        cited_fragments = " ".join(
            _format_cited_sentence(document.text, document.doc_id)
            for document in evidence[:2]
        )
        prompt = f"Question: {question}\nEvidence: {cited_fragments}"
        _ = self.llm.complete(prompt)
        answer = f"Based on the retrieved research evidence, {cited_fragments}"
        return GeneratedAnswer(
            question=question, answer=answer, citations=citations, evidence=evidence
        )
